...
class BiorxivScraper:
    """bioRxiv预印本平台爬虫，基于paperscraper项目格式重写"""
    ...
    def __init__(self):
        ...
        self.api_papers_url = f"{self.api_base_url}/details/biorxiv/{{start_date}}/{{end_date}}/{{cursor}}"
        self.headers = {
        ...
        self.launch_date = "2013-01-01"  # bioRxiv启动日期
    ...
    def _call_api(self, start_date, end_date, cursor=0):
        ...
        logger.info(f"调用bioRxiv API: {url}")
    ...
    def _get_papers_from_api(self, start_date=None, end_date=None, limit=None):
        ...
        logger.info(f"从bioRxiv API获取论文: 日期范围={start_date}至{end_date}, 限制={limit}")
        ...
        try:
            ...
            logger.info(f"从bioRxiv API获取到 {len(papers)} 篇论文")
            return papers
            
        except Exception as e:
            logger.error(f"从bioRxiv API获取论文时出错: {str(e)}")
            import traceback
    ...
    def _process_api_paper(self, paper_data):
        ...
        try:
            ...
            # 检查本地缓存
            paper_id = f"biorxiv_{doi.replace('/', '_').replace('.', '_')}"
    ...
    def _extract_paper_data(self, article_elem):
        """从文章元素中提取论文数据
        
        参数:
            article_elem: BeautifulSoup元素，表示一篇文章
            
        返回:
            dict: 论文数据字典，如果提取失败则返回None
        """
        try:
            # 提取标题
            title_elem = article_elem.select_one('.highwire-cite-title, h2 a, .title a')
            if not title_elem:
                return None
            ...
            # 提取DOI
            doi_elem = article_elem.select_one('.highwire-cite-metadata-doi')
            doi = ""
            if doi_elem:
                doi_text = doi_elem.get_text(strip=True)
                doi_match = re.search(r'10\.\d+/[\w\.\-/]+', doi_text)
                if doi_match:
                    doi = doi_match.group(0)
            
            if not doi:
                # 如果找不到DOI，从链接中提取或生成一个
                doi_match = re.search(r'10\.\d+/[\w\.\-/]+', article_link)
                if doi_match:
                    doi = doi_match.group(0)
                else:
                    import hashlib
            ...
            # 提取作者
            authors = []
            author_elem = article_elem.select_one('.highwire-citation-authors, .meta-authors')
            if author_elem:
            ...
            # 提取日期
            published = datetime.now().strftime("%Y-%m-%d")  # 默认为当前日期
            date_elem = article_elem.select_one('.highwire-cite-metadata-date, .meta-date')
            if date_elem:
                date_text = date_elem.get_text(strip=True)
                published = self._format_date(date_text)
            
            # 提取摘要
            abstract = ""
            abstract_elem = article_elem.select_one('.abstract, .highwire-cite-abstract')
            if abstract_elem:
                abstract = abstract_elem.get_text(strip=True)
            
            # 构建PDF URL
            pdf_url = f"{article_link}.full.pdf"
            ...
            return {
                ...
                'local_pdf_path': local_pdf_path if has_local_cache else None,
                'full_text': title + " " + abstract  # 用于关键词匹配
            }
            
        except Exception as e:
            logger.error(f"提取论文数据时出错: {str(e)}")
            return None
    ...
    def fetch_papers(self):
        ...
        try:
            ...
            logger.info(f"从bioRxiv获取论文: {start_date} 至 {end_date}")
            papers = self._get_papers_from_api(start_date, end_date, MAX_RESULTS)
            
            if not papers or len(papers) == 0:
                logger.warning("未从bioRxiv API获取到论文，尝试使用搜索方式")
            ...
            logger.info(f"从bioRxiv获取到 {original_count} 篇论文，过滤后剩余 {len(papers)} 篇")
            return papers
            
        except Exception as e:
            logger.error(f"获取bioRxiv论文时出错: {str(e)}")
            import traceback
            logger.error(traceback.format_exc())
            return []